...
class DualUR5e(Robot):
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        left_action = {k.replace("left_", "", 1): v for k, v in action.items() if k.startswith("left_")}
        right_action = {k.replace("right_", "", 1): v for k, v in action.items() if k.startswith("right_")}
    ...
    def stop_force(self) -> None:
        ...
        self.left_arm.stop_force()
        self.right_arm.stop_force()
