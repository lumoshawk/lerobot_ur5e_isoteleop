...
class DynamixelDriver(DynamixelDriverProtocol):
    ...
    def _initialize_with_retries(self) -> bool:
        ...
        for attempt in range(self._max_retries):
            logger.info(
                f"[TELEOP] Attempting to initialize Dynamixel driver (attempt {attempt + 1}/{self._max_retries})"
            )

            # Check port availability
            if not self._check_port_availability():
                logger.info("[TELEOP] Port is busy, attempting to free it...")
                if not self._kill_processes_using_port():
                    logger.info("[TELEOP] Failed to free port, trying to fix permissions...")
                    self._fix_port_permissions()
                time.sleep(2)

            try:
                self._initialize_hardware()
                logger.info(f"[TELEOP] Successfully initialized Dynamixel driver on {self._port}")
                return True
            except Exception as e:
                logger.info(f"[TELEOP] Failed to initialize Dynamixel driver: {e}")
                if attempt < self._max_retries - 1:
                    logger.info("[TELEOP] Retrying in 2 seconds...")
                    time.sleep(2)
                else:
                    logger.info("[TELEOP] Max retries reached")
